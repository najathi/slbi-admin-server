...
if (!empty($_POST)) {
    ...
    $nic_b_fileNameFull = uniqid("", true) . '1' . $nic_b_fileName;

    // image file directory
    $target = '../../assets/images/nic-photo/';
    $target_nic_f_img = $target . basename($nic_f_fileNameFull);
    $target_nic_b_img = $target . basename($nic_b_fileNameFull);

    // delete already exist file
//    $select_query_file = "SELECT * FROM sim_details WHERE id = '$id'";
...
//    }
    ...
    $query = "UPDATE sim_details SET namee = '$namee', address_l_one = '$address_l_one', address_l_two = '$address_l_two', nic_no = '$nic_no', nic_f_img = '$nic_f_fileNameFull', nic_b_img = '$nic_b_fileNameFull', m_no = '$m_no', designation = '$designation', gn_div = '$gn_div', ds_div = '$ds_div', district = '$district', sim_no = '$sim_no', sim_s_no = '$sim_s_no', user_id = '$user_id', updated_at = '$updated_at' WHERE id = '$id'";
...
//    echo $resUpdate;
    ...
    if ($resUpdate) {
        ...
        $select_query = "SELECT * FROM sim_details";
        ...
        while ($row = mysqli_fetch_array($result)) {
            $output .= '
                    ...
                    <tr>
                        ...
                        <td>' . $row["sim_no"] . '</td>  
                        <td>
                        <a style="cursor: pointer;" class="text-secondary viewBtn" id="' . $row['id'] . '" data-toggle="modal tooltip" data-target=".view_sim_details" data-whatever="' . $row['id'] . '" data-placement="top" title="View"><i class="fa fa-eye"></i></a> &nbsp;&nbsp;|&nbsp;&nbsp;
    <a style="cursor: pointer;" class="text-secondary editBtn" id="' . $row['id'] . '" data-toggle="modal" data-target="#edit_sim_details" data-placement="top" title="Edit"><i class="fa fa-edit"></i></a> &nbsp;&nbsp;|&nbsp;&nbsp;
    <a class="text-danger delete" id="' . $row['id'] . '" data-toggle="tooltip" data-placement="top" title="Delete"><i class="ti-trash"></i></a>
                        </td>
                        
                        <td>
                        <a href="pdf/pdf_sim_details?id=' . $row['id'] . '" target="_blank" class="supp_pfd" style="cursor: pointer;" data-toggle="tooltip" id="' . $row['id'] . '" data-placement="top" title="Receipt"><i class="fa fa-print"></i></a>
                        </td>
                    </tr>
                  </tbody>
              ';
